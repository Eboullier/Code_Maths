...
class HilbertCurve(VMobject):
    ...
    def __init__(self, **kwargs):
        ...
        if self.z_index == 1:
            ...
            curves = VGroup()
            for i in range(4):
                order = self.z_index - 1
                curve = HilbertCurve(z_index=order)
...
class HilbertCurveScene(MovingCameraScene):
    def construct(self):
        n = 2  # order of the curve
        offset = 1.1  # offset wrt height of the camera frame
        colors = (BLUE,RED)

        old_hilbert = HilbertCurve(z_index=1)
        old_hilbert.set_height(self.camera.frame.get_height() / offset)

        final_hilbert = HilbertCurve(z_index=n)
        colors = color_gradient(colors, len(final_hilbert.get_points()))

        order_is = text, number = VGroup(
            Tex("Order = ", font_size=48),
        ...
        for i in range(2, n + 2):
            ...
            if i == n:
                ...
                new_hilbert = HilbertCurve(z_index=order_tracker.get_value() + 1)
            new_hilbert.set_height(self.camera.frame.get_height() / offset)

            self.play(
                old_hilbert.animate.rotate, PI / 2,
