...
class roll(MovingCameraScene):
    def construct(self):
        ...
        congtext=VGroup(Text("Thus, these two triangles are similar:", font_size=thesize, font="Arial"),MathTex("{{\\frac{\\pi r}{x}}}").scale(.6),Tex("{{=}}").scale(.6),MathTex("{{\\frac{x}{r}}}").scale(.6))

        congtext.arrange(RIGHT)
        congtext.move_to((-1.5,-2.5,0))
        ztext=MathTex("{{x^2=\pi r^2}}").scale(.6)
        ...
        circarea=MathTex("A=\\pi r^2").scale(.7)
        ...
        semicirc = ParametricFunction(
            lambda u: (-PI/2+math.cos(PI/2-u),1+math.sin(PI/2-u),0), t_range = [0,PI]) 
        semicirc.set_stroke(color2, width=4)
        self.play(Create(radius))

        self.play(Create(unitcirc))
        self.play(Create(topdot))
        ...
        def update_semi(mob, alpha):
            mob.become(
                ParametricFunction(
            lambda u: (interpolate(0,PI,alpha)-PI/2+math.cos(PI/2-u-interpolate(0,PI,alpha)),1+math.sin(PI/2-u-interpolate(0,PI,alpha)),0), t_range = [0,PI])
            )

        self.play(
            UpdateFromAlphaFunc(unitcirc, update_circ),UpdateFromAlphaFunc(topdot, update_dot),UpdateFromAlphaFunc(radius, update_rad),UpdateFromAlphaFunc(semicirc, update_semi),Create(BigRadius),**play_kw
        )

        bradbracelabel=MathTex("\\pi r").scale(.7)
        ...
        Bigsemicirc = ParametricFunction(
            lambda u: (.5+(1+PI/2-.5)*math.cos(u),(1+PI/2-.5)*math.sin(u),0), t_range= [-PI,0]) 
        ...
        self.play(Create(thales1))
        self.play(Create(thales2))
        ...
        resultx=MathTex("x^2=\pi r^2").scale(.7)
        ...
        self.play(Create(TheSquare), **play_kw)

        self.wait(2)


        sqarea=MathTex("A=x^2").scale(.7)
        ...
        sqarea2=MathTex("=\pi r^2").scale(.7)
