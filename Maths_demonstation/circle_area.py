...
class Demo(Scene):
    def construct(self):
        ...
        self.play(c_group.animate.shift(3*UP))
        ...
        for i in lst:
            for j in range(i):
                annulus = Annulus(j * radius / i, (j + 1) * radius / i, stroke_width=2) \
                    .set_fill(BLUE, opacity=0).move_to(circle)
                ...
                if i != 20:
                    self.play(DrawBorderThenFill(annulus))
            
            if i == 20:
                self.bring_to_back(annulus_g)
                self.play(DrawBorderThenFill(annulus_g))
        ...
        r_tex2 = MathTex("r") \
            .next_to(rectangles, UP).shift(1.5*RIGHT)
        arrow = CurvedArrow(radius_line.get_end(), r_tex2.get_right())
        r_tex2.shift(0.3*LEFT)
        ...
        area1 = MathTex(r"A = {2\pi r \cdot r \over 2}").shift(2.7*DOWN)
        area2 = MathTex(r"A = ", r"{2", r"\pi r^2 \over", r"2}").shift(2.7*DOWN)
        cancels = VGroup(
            CancelLine(area2[1]),
            CancelLine(area2[3])
        )
