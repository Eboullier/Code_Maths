...
class ManimScene(Scene):
    ...
    def construct(self):
        x_g, y_g = self.get_grid()
        context = VGroup(x_g, y_g)
        context.set_height(config.frame_height-0.5)
        context.move_to(ORIGIN)

        angular_freq_label = MathTex("\\omega")
        ...
        self.play(
            *[
                ApplyMethod(xvt.set_value, val*1)
                for xvt,val in zip(x_vtrackers, x_trackers_target)
            ],
            *[
                ApplyMethod(yvt.set_value, val*1)
        ...
        black_rect = Rectangle(3,3,fill_opacity=1,fill_color=BLACK,stroke_color=BLACK)\
            .set_width(config.frame_width,stretch=True)\
            .set_height(config.frame_height,stretch=True)
